...
// Define list of supported HTTP request methods
var methods = ["get", "post", "put", "delete"];

/**
 * Extend http.Server prototype
 */

// Store server routing tree
http.Server.prototype._routes = {};
 ...
 * @return {void}
 */
http.Server.prototype._router = function(request, response) {
 ...
 */
http.Server.prototype._routeRequest = function(input) {
	...
	if (typeof input.segments !== "undefined" &&
		...
		if (input.segments.length == 0) {
			...
			if (typeof handler === "undefined" && typeof route[":var"] !== "undefined") {
 ...
 */
http.Server.prototype._addRoute = function(method, route, callback) {

	// Validate route input
	if (typeof route !== "string") {
		throw new URIError("Invalid Route.");
	}
...
methods.forEach(function(method) {
	http.Server.prototype[method] = function(route, callback) {
		this._addRoute(method.toUpperCase(), route, callback);
	}
})
...
ZeeServer = function() {
	...
	server.on("request", server._router);

	return server;

}

module.exports = ZeeServer;